...
import ArtesImage from "/Artes1Card.png";
...
const Artes = () => {
  ...
  const [reviews, setReviews] = useState([
    ...
    { id: 1, usuario: "PintorLoco", estrellas: 5, texto: "¡El café es mi inspiración! Excelente servicio.", fecha: "15/11/2025" },
    { id: 2, usuario: "EscultorNovato", estrellas: 3, texto: "Buen precio, pero la fila es eterna en la hora pico.", fecha: "14/11/2025" }
  ]);

  // Cargar Menú
  useEffect(() => {
    fetch("http://localhost:8000/api/Menus/?id_tiendita=10")
      .then(response => {
  ...
  const handleSubmitResena = (e) => {
    ...
    // Crear objeto de reseña (Simulación por ahora)
    const nuevaResena = {
      id: Date.now(),
    ...
    // Agregar a la lista (Aquí iría el POST al backend luego)
      ...
      <main style={{ flexGrow: 1, padding: "2rem", paddingTop: "7rem" }}>
        ...
        <div style={{
          ...
          {loading ? (
            ...
            <p style={{ textAlign: "center" }}>No hay elementos en el menú.</p>
          )}
        </div>
